...
*/
const FORK_SCOPES = ["@x1a0f3n9/", "@xfcodeai/"];
...
function withForkAliases(officialNames) {
	const names = [];
	for (const officialName of officialNames) {
		const rest = officialName.slice(13);
...
*/
const RUNNING_PROBE_PACKAGES = withForkAliases(["@deepseek-ai/dsh-session", "@deepseek-ai/dsh-session-projection"]);
/**
...
function equivalentPackageNames(packageName) {
	let rest;
	if (packageName.startsWith(OFFICIAL_SCOPE)) rest = packageName.slice(13);
	else for (const scope of FORK_SCOPES) if (packageName.startsWith(scope)) {
		rest = packageName.slice(scope.length);
		break;
	}
	if (rest === void 0) return [packageName];
	return [OFFICIAL_SCOPE + rest, ...FORK_SCOPES.map((scope) => scope + rest)];
}
function packageNamesMatch(recordName, expectedName) {
...
function versionOfManifest(manifestPath, expectedName) {
	try {
		...
		const record = parsed;
		if (expectedName !== void 0 && !packageNamesMatch(record.name, expectedName)) return void 0;
